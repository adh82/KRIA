...
function Prms:add_globals()
	...
	data:add_binary('midi_scale_quantize', 'MIDI SCALE QUANTIZE', 'toggle', 1)
	...
	data:add_option('loop_sync', 'LOOP SYNC', loop_sync_modes)
	data:set_action('loop_sync', function() sync_step_count() end)
...
function Prms:add_tracks()
	...
	for t = 1, NUM_TRACKS do
		...
		for k, v in pairs(pages_with_steps) do
			local page = track[v]
			page:add_number('pos', 'data', 1, 64, 1)
